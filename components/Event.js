    ...
    <div className="mb-10">
      <div key={id} className="md:flex md:items-center">
        <div className="flex items-center gap-x-2 mb-2 md:flex-1 md:gap-x-4">
          <KeyboardArrowDownIcon className={`cursor-pointer transition ease-out ${showDetails && "rotate-180"}`} onClick={() => setShowDetails(!showDetails)} />
          <div
          ...
          <div className="ml-auto md:ml-0 md:ml-10">
            <h3 className="font-bold text-sm truncate max-w-[200px] overflow-hidden md:max-w-[270px] md:text-base lg:max-w-full">{name}</h3>
      ...
      {showDetails && <EventDetails priceMin={priceMin} priceMax={priceMax} />}
    </div>
  );
};
